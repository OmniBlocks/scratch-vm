...
class Runtime extends EventEmitter {
    constructor () {
        ...
        // Add these new properties in the constructor
        this._extensionAudioObjects = new Map();
        this._extensionVariables = {};
        this.cameraStates = {};
        
        // Also update/add the serializers object:
        this.serializers = {
            'pm-rendered-target': {
            ...
            'pm-costume-asset': {
                serialize: costume => ({ assetId: costume.assetId, name: costume.name }),
                deserialize: ({ assetId, name }) => this.getCostumeByAssetId(assetId) ?? this.getCostumeByName(name)
            }
        };
    }
    ...
    /**
     * Event emitted when camera state changes
     * @const {string}
     */
    Runtime.CAMERA_CHANGED = 'CAMERA_CHANGED';
        /**
     ...
     */
    /**
 * Register compiled extension blocks for the compiler
 * @param {string} extensionId - The extension ID
 * @param {object} information - Object containing ir and js properties
 */
...
/**
 * Register a serializer for extension data in variables
 * @param {string} id - Serializer ID
 * @param {Function} serialize - Serialization function
 * @param {Function} deserialize - Deserialization function
 */
 ...
 * @param {Function} varClass the variable class constructor
 */
...
newVariableInstance (type, ...args) {
    const coreVariableTypes = ['', 'list', 'broadcast_msg'];
...
/**
 * Get branch and target by block ID and branch ID
 * @param {string} id - Block ID
 * @param {string} branchId - Branch ID
 * @returns {Array|null} [branch, target] or null if not found
 */
 ...
 * @param {string} screen the screen that changed
 */
